...
header('Access-Control-Allow-Headers: Origin, Content-Type, X-Requested-With, Authorization'); // allow these request headers
...
//echo "Matching rows: " . $count;
    ...
    if ( $count > 0) {
      //  echo "row existed";
         $update_sql = "UPDATE `adding-bed-details1` SET `amount` = '$fee',`no_of_persons_sharing_per_room` = '$no_of_persons_sharing_per_room'
         ,`bed_no` = '$bed_no' 
          WHERE `floor_no` = '$floor_no' AND `room_no` = '$room_no' 
           AND `building_name` = '$building_name' AND `manager_email` = '$manager_email'";
            //echo $update_sql;
